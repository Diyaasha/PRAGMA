...
export default function ResolutionFunnel({ maps }) {
  ...
  return (
    <section className="rounded-xl border border-line bg-white dark:bg-card p-5">
      <p className="mb-5 font-mono text-[10px] uppercase tracking-[0.18em] text-brass-deep dark:text-brass">Resolution Funnel</p>
      <div className="space-y-3.5">
        {stages.map((s, i) => (
          <div key={s.label}>
            <div className="mb-1 flex items-baseline justify-between">
              <span className="text-sm font-medium text-ink dark:text-[#e8edf5]">{s.label}</span>
              <span className="font-mono text-[11px] text-[#8b98aa]">{s.hint}</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="h-7 flex-1 overflow-hidden rounded-md bg-line dark:bg-surface">
                <div
                  className="flex h-full items-center justify-end rounded-md px-2 transition-all"
                  style={{
                    width: `${Math.max(pct(s.value), 8)}%`,
                    background: `rgba(20,48,73,${0.9 - i * 0.16})`,
                  }}
                >
                  <span className="font-serif text-sm font-semibold text-white tabular-nums">{s.value}</span>
                </div>
              </div>
              <span className="w-10 flex-shrink-0 text-right font-mono text-[11px] tabular-nums text-[#8b98aa]">{pct(s.value)}%</span>
            </div>
          </div>
        ))}
      </div>
      <p className="mt-4 border-t border-line pt-3 text-xs text-[#8b98aa]">
        {total - reviewed > 0
